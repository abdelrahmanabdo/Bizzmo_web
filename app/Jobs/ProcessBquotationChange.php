...
class ProcessBquotationChange implements ShouldQueue
{
	...
	public function handle()
	{
		...
		// Process bizzmo quotation mail to buyer		
		$bquotation = $this->quotation->vendornumber;
		$data = ['company' => $this->quotation->company->companyname, 'quotation' => $this->quotation];
		Mail::send('emails.bquotationtobuy', $data, function ($message) use ($path, $filename, $bquotation) {
			$message->subject('Your Bizzmo Offer# ' . $bquotation . ' has been modified!');
			$message->to($this->quotation->company->email);
			$message->bcc(env('MAIL_ARCHIVE'));
			$message->attach($path . $filename);
		});
		
		// Process bizzmo quotation mail to supplier		
		...
		Mail::send('emails.bquotationtosup', $data, function ($message) use ($squotation) {
			$message->subject('Your Quote # ' . $squotation. ' has changed!');
