...
@section('content')
	@if (isset($purchaseorder)) 
		{{ Form::model($purchaseorder, array('id' => 'frmManage', 'class' => 'po-form')) }}
	@else
	...
	<div class="row flex-container bm-pg-header">
		<h2 class="bm-pg-title">Create Purchase Order</h2>
	</div>
	@endif
				...
				@else
					<div class="row">
						<div class="col-xs-8" style="padding-right: 0px">
							{{ Form::select('vendor_id', $vendors, old('vendor_id'),array('id' => 'vendor_id', 'class' => 'form-control bm-select'))}}		
		...
		@if (isset($purchaseorder) && isset($purchaseorder->quotation))
			<div class="col-md-3">  <!-- column 4 -->
				<div class="form-group"> <!-- supplier -->  
					{{ Form::label('qu', 'Quotation') }}
					@php $quView = $purchaseorder->quotation->userrelation == 1 ? 'bview' : 'view' @endphp
					<br><a href="/quotations/{{ $quView }}/{{$purchaseorder->quotation->id}}" class='form-control-static'>#{{ $purchaseorder->quotation->number }} (ver. {{ $purchaseorder->quotation->version }})</p>
				</div> <!-- supplier -->
			</div>
		@endif
	</div>
				...
				<tfoot>
					...
					@if (isset($purchaseorder) && $purchaseorder->userrelation != 2)
						...
						<tr>
							...
							<td>
								@if (isset($purchaseorder))
								VAT {{ $purchaseorder->vat }}<span> %</span>
									{{ Form::hidden('VAT', $purchaseorder->vat, array('id' => 'VAT')) }}
								@else
									VAT - {{ $company->vat }}<span> %</span>
									{{ Form::hidden('VAT', $company->vat, array('id' => 'VAT')) }}
								@endif								
							</td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
			...
			$("#shippingaddress_id").change(function(){
				...
				$.ajax({					
                    ...
                    success: function(response){
						//console.log('aa' + response.city.cityname);
